...
def rad_cal_mat(cover,cam_id,exposure,DN):
    ...
    dir1 = f"/calibration/calibration params/radcal_params/"
...
def ff_correct(image, cam_id, exposure):
    dir_ff1 = f"/calibration/calibration params/radcal_params/"
    dir_ff = os.path.abspath(os.path.dirname(__file__)+dir_ff1)

    dir_dark1 = f"/calibration/calibration params/DarkNoise"
    dir_dark = dir_dark1+ f"/camera_{cam_id}/DarkNoise_CAM{cam_id}_{exposure}ET.npy"
    dir_dark = os.path.abspath(os.path.dirname(__file__)+dir_dark )


    dark_cover = np.load(dir_dark)
...
def main():
    ...
    cam_id = ID[cam]
    exposure = 3000
    dir1 = r'/calibration/101934/full_scan/polcal_40_101934.npy'  #for example
    dir= os.path.abspath(os.path.join(os.path.dirname(__file__), dir1))
