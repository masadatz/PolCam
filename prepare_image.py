...
def prepare_image(image_dir, use_pol_cal=True):
    ...
    # load original image
    image_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), image_dir))
    ...
    if cam_id == '192900073':
        ...
        for i in range(3):
            ...
            if (use_pol_cal):
                ...
                Stokes_rad_full= Stokes_rad

            # correct distortion get camera position
            Stokes_dist_correction = GeoCal.intrinsic(Stokes_rad_full, CALIB_DAY, cam_id)

            # Add mask
            mask_dir1 = f"/Data_From_Experiment/masks/{CALIB_DAY}/{cam_id}.png"
            mask_dir = os.path.abspath(os.path.dirname(__file__) + mask_dir1)
...
def main():
    dir = f"Data_From_Experiment/B/B42_25000/10"
    directory = os.path.abspath(os.path.join(os.path.dirname(__file__), dir))
    for file in os.listdir(directory):
        if (file.endswith(".png") or file.endswith(".npy") ):
            image_dir = dir+"/"+file
            Stokes_final = prepare_image(image_dir,False)
            cam_id = image_dir.split('_')[-1].split('.')[0]
            save_dir = directory + "prepared"
            if not os.path.exists(save_dir):
                 os.mkdir(save_dir)
            np.save(save_dir+"/"+ cam_id,Stokes_final)
