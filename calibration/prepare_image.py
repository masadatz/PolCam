...
def prepare_image(image_dir1):
    ...
    if cam_id == '192900073':
        ...
        for i in range(3):
            ...
            Stokes_final[:, :, :, i] = Stokes_dist_correction
        ...
        Stokes_final = Stokes_dist_correction
...
def main():
    dir =f"C:/Users/masadatz/Google Drive/CloudCT/svs_vistek/Data_From_Experiment"
    image_dir= dir+f"/B/B42_25000/8/26_10_45_50_386804_101936.png"
    cam_id = image_dir.split('_')[-1].split('.')[0]
    exposure = int(image_dir.split('/')[-3].split('_')[1])
    bit = [12 if 'A' in image_dir else 8]
    CALIB_DAY = find_geocal_day(image_dir)
    ...
    if cam_id == '192900073':
        ...
        for i in range(3):
            ...
            Stokes_final[:,:,:,i]=Stokes_dist_correction
            DoLP[:,:,i] = pa.cvtStokesToDoLP(Stokes_final[:,:,:,i])
        rgb_DoLP = Image.fromarray((DoLP*256).astype(np.uint8), "RGB")
        rgb_DoLP.show()
    ...
    #add mask TODO
    np.save(f"C:/Users/masadatz/Google Drive/CloudCT/svs_vistek/Data_From_Experiment/Stokes_cal_{cam_id}_B42_8_corrected.npy",Stokes_final)
